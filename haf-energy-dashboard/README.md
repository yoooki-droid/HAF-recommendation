...
`POST /api/energy-reading` 只接收已经计算完成的匿名能量事实，不接收生日、性别或城市。当前 v4 请求以用户松手锁定的共鸣词、该词对应的主能量、辅助能量与今日主旋律为依据，不再接收旧罗盘方向字段。后端使用 DeepSeek 将这些事实润色为 18–50 个中文字符、灵性但不玄断、温暖而有力量的解读，并按匿名用户、自然日和结果签名缓存。同一次结果不会重复调用；接口失败时 H5 使用本地规则文案。H5 在合成 Loading 阶段发起请求，最长等待 6 秒，进入结果页后不再更新文字。
