...
- The experience is intended to sit inside the mini-program homepage, so app-owned H5 content is presented as a 700px-tall embedded module rather than filling the whole phone screen.
...
- The 700px H5 module spans the full available phone width with square outer edges. Do not add a floating rounded outer frame or horizontal gutters around the whole module; rounded corners remain appropriate for internal cards and controls.
- Result-page readability takes priority over extra negative space: `灵数 · 脉轮 · 方向`, course fit copy, duration/type, and bookmark labels must remain comfortably readable at the 365px embedded H5 width. Include a concise `今日能量总结` below the primary insight.
- Match the host mini-program’s compact PingFang-style hierarchy: approximately 31-34px for an exceptional hero/result phrase, 25px for screen titles, 17px for section/course titles, 12px for readable body copy, 10px for captions, and 9px only for tertiary metadata. Prefer regular-to-semibold weights (400-650); avoid oversized heavy display type that makes the embedded H5 feel like a separate product.
- The result recommends exactly three courses in a horizontal carousel. Every card includes a small real image, course name, today's fit reason, duration/format, and a bookmark action. Do not show price or purchase pressure.
...
- `今日能量总结` is a visible secondary heading, not microcopy. The saved-count action opens a dedicated saved-experiences screen, and that screen must remain part of the flow.
