...
“探索本心”是嵌入 HAF 小程序首页的 700px 高 H5 模块。它把用户的出生数字线索、当天自然日节律、手指在四象罗盘上的当前感应，以及 HAF 七脉轮反思模型结合，生成当日主旋律、能量解读和三节课程推荐。
...
- 结果页保留今日主旋律、此刻入口、能量落点和一句有判断的总结
- 恰好三节课程的横向推荐，包含真实小图、名称、契合理由、时长和形式
- 本地收藏、取消收藏和独立收藏页
- “重新感应”返回已有罗盘，不产生重复路由或死循环
- 访问与成功收藏的最小匿名埋点
- 独立轻量数据看板
...
| 罗盘 | 连续 x/y 坐标 | 向内/向外、安静/行动与强度 |
| 脉轮模型 | 数字亲和度、罗盘位置 | 主脉轮、次脉轮与主题 |
| 课程推荐 | 上述结构化结果、课程标签 | 三节课程及确定性契合理由 |

相同用户、自然日和结果签名保持稳定；新的自然日、出生计算结果或罗盘落点可以产生新内容。
...
`npm run test:runtime` 需要本机已安装 Playwright Chromium。无该依赖时，仍需在内嵌手机预览中手动走完首次、回访、修改资料、罗盘两种方向、重新感应、三张课程卡和收藏页。
