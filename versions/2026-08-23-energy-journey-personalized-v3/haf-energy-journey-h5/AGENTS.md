...
- Match the host mini-program’s compact PingFang-style hierarchy. In the approved v3 embedded module, the largest heading is 24px, section/course titles are approximately 17-18px, normal body copy is 14px, and non-course-card captions are no smaller than 12px. Prefer regular-to-semibold weights (400-650); avoid oversized heavy display type that makes the embedded H5 feel like a separate product.
- For the current mobile-embedded typography pass, all app-owned text outside the final recommendation course cards must be at least 12px, normal body copy uses 14px, and no heading exceeds 24px. The returning-screen welcome line and its daily greeting headline share the same 24px size. Keep the recommendation course-card typography unchanged until the user asks to revise it separately.
