...
## 0. v4 当前实现摘要（2026-08-27 修订）

> 本文后续保留的“四象罗盘”章节仅用于记录旧方案。当前产品已经改为“用户选词反推脉轮”，不得继续用上下左右坐标推测主脉轮。

- 已实现 `haf-numerology`、`haf-chakra-energy`、`haf-energy-synthesis`、`haf-course-recommendation` 四个项目 Skill。
- 当前感应面是隐藏的 7 × 10 词场：七个脉轮各十个词，位置只负责寻址，不再代表向内、向外、安静或行动。
- 词不会按时间轮播；只有进入新的位置单元才会变化。松手锁定当前词，该词直接决定主脉轮（70%），Life Path（20%）与 Personal Day（10%）只提供支持信号与次脉轮。
- 课程推荐必须先匹配用户选词对应的主脉轮，再结合词语标签、当日灵数、练习投入度与新鲜度排序。
...
5. 解释每节课“为什么与此刻的你契合”，允许用户先收藏或购买、未来再进入，不制造“今天必须上课”的压力。
