...
- The core flow is spiritual loading -> one-time profile -> full-screen intuitive sensing -> synthesis -> daily keyword and three course recommendations.
- The sensing screen never exposes axes, seven points, or chakra terminology. It uses `public/assets/haf/visual-refresh/intuitive-flow-field-v1.png` as its real full-screen visual field, a Canvas trail/ripple layer, and the existing energy orb. Press/hold time, movement speed, movement range, pointer pressure when available, and continuous position shape the existing versioned HAF compass point; releasing locks the current word. Keep the seven HAF chakra dimensions, numerology weights, and primary/secondary projection in the backend model unchanged. The locked `完成感应` action is a translucent 52px glass pill matching the result screen's `换一批` treatment, never a solid blue button. Honor reduced-motion preferences by keeping the field static and suppressing trails/ripples.
...
- Recommendation cards must retain rounded top and bottom corners even when blurred copy layers use GPU compositing. The three energy-facet footer values (`力量`, direction detail, and chakra themes) share one aligned baseline regardless of the different main-value sizes.
- Course recommendation eligibility is gated by the displayed primary chakra: every returned course must include that chakra in `chakra_tags`. `换一批` exhausts unseen courses only inside this relevant pool, avoids the immediately previous batch when possible, and recycles older matching courses before ever introducing a course tagged only for another chakra. Relevance always outranks novelty.
