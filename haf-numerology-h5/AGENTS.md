...
## HAF Prototype Decisions

- Keep the selected “inner compass” visual direction: festival blue atmosphere, translucent glass layers, and one blue-orange light orb as the primary interaction object.
- Treat Numerology as an optional playful reflection lens, never as factual prediction or a deterministic course-ranking signal.
- The repeat loop is daily keyword → current energy compass → chakra insight → desired compass → course collection → inner trajectory; favorites and trajectory persist locally across plays.
- Course conversion should surface repeated user needs and invite a light first experience, without fear, scarcity, or immediate bundle pressure.
- V2 should collect the birthday and explain the journey warmly, but must not ask preliminary mood questions when the energy compass already captures the user's current state.
- Treat the energy compass, seven chakras, and numerology as three peer reflection tools. Numerology may provide a daily keyword, but its number should remain visually secondary.
- Use energy language rather than “inner weather.” Present chakra and numerology content as symbolic, actionable reflection rather than medical, psychological, or deterministic prediction.
- The two approved visual anchors are the full-height blue glass energy-choice card and the large draggable energy compass. Match their spacious white typography, translucent deep-blue panel, orange selected glow, and blue-orange flowing light field.
- Keep the daily loop visually intuitive and short: one energy choice → one draggable compass → one concise insight with one recommended course. Avoid explanatory dashboards, method lists, percentage logic, and long instructional copy.
